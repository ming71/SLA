...
def main(rank, world_size):
    ...
    dir_save = 'weights'
    hyp = 'run/ucas_aod/hyp.py'

    hyps = hyp_parse(hyp)

    dir_weight = os.path.join(dir_save, 'ucas_weight')
    dir_log = os.path.join(dir_save, 'ucas_log')
    ...
    lr_cfg = [[0.7 * max_step, lr], [max_step, lr / 10]]
    ...
    dataset = UCAS_AOD(data_dir, ['train'], aug)
    ...
    loader = DataLoader(dataset, batch_sampler=batch_sampler, num_workers=int(hyps['num_workers']), collate_fn=dataset.collate)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Training for UCAS_AOD dataset...')
