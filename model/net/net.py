...
from .utils.cooratten import CoordAtt


class Net(xnn.Module):
    def __init__(self, backbone, cfg):
        ...
        # self.coorattn = nn.ModuleList()
        # for i in range(self.num_levels):
        #     self.coorattn.append(CoordAtt(self.fpn_plane, self.fpn_plane))
    ...
    def forward(self, images, targets=None, process=None):
        ...
        # a_features = []
        # for i, p_feature in enumerate(p_features):
        #     a_features.append(self.coorattn[i](p_feature))
        # pred_cls, pred_loc = self.predict(a_features)

        pred_cls, pred_loc = self.predict(p_features)
        anchors = self.prior_box.get_anchors(images.shape[2:]).to(images)
        if self.training:
            if targets is not None:
                return calc_loss(pred_cls, pred_loc, targets, anchors, self.iou_thresh, self.variance, self.balance, process)
        else:
